...
export const generateLessonPlan = async (
  ...
  try {
      ...
      for (let i = 0; i < cleaned.length; i++) {
        ...
        if (inString) {
          ...
          if (char === '.') {
            let dotCount = 1;
            let j = i + 1;
            while (j < cleaned.length && cleaned[j] === '.' && dotCount < 1000) {
              dotCount++;
              j++;
            }
    ...
    try {
      ...
      // Example: "nhan_xe\n    t": -> "nhan_xet":
      jsonText = jsonText.replace(/"([a-z0-9_]+)"\s*\n\s*"([a-z0-9_]{1,5})"\s*:/gi, (match, p1, p2) => {
      ...
      for (let i = 0; i < jsonText.length; i++) {
        ...
        if (char === '"') {
          if (inStringFix) {
            ...
            if (j < jsonText.length && (jsonText[j] === ':' || jsonText[j] === ',' || jsonText[j] === '}' || jsonText[j] === ']')) {
              // Valid closing
              inStringFix = false;
              fixedJson += char;
            } else {
      ...
      fixedJson = fixedJson.replace(/([,\{]\s*)"([a-z0-9_]+)"\s+(")/gi, '$1"$2": $3');
      fixedJson = fixedJson.replace(/([,\{]\s*)"([a-z0-9_]+)"\s+(\{)/gi, '$1"$2": $3');
      fixedJson = fixedJson.replace(/([,\{]\s*)"([a-z0-9_]+)"\s+(\[)/gi, '$1"$2": $3');
      fixedJson = fixedJson.replace(/([,\{]\s*)"([a-z0-9_]+)"\s+(-?\d+)/gi, '$1"$2": $3');
      fixedJson = fixedJson.replace(/([,\{]\s*)"([a-z0-9_]+)"\s+(true|false|null)/gi, '$1"$2": $3');
      ...
      const isMissingCommaError = parseError.message?.includes("Expected ','") || 
                                 parseError.message?.includes("Expected '}'");
      const isMissingColonError = parseError.message?.includes("Expected ':'") || 
      ...
      if (isMissingColonError) {
        ...
        jsonTextForFix = jsonTextForFix.replace(/([,\{]\s*)"([a-z0-9_]+)"\s+(")/gi, '$1"$2": $3');
        jsonTextForFix = jsonTextForFix.replace(/([,\{]\s*)"([a-z0-9_]+)"\s+(\{)/gi, '$1"$2": $3');
        jsonTextForFix = jsonTextForFix.replace(/([,\{]\s*)"([a-z0-9_]+)"\s+(\[)/gi, '$1"$2": $3');
        jsonTextForFix = jsonTextForFix.replace(/([,\{]\s*)"([a-z0-9_]+)"\s+(-?\d+)/gi, '$1"$2": $3');
        jsonTextForFix = jsonTextForFix.replace(/([,\{]\s*)"([a-z0-9_]+)"\s+(true|false|null)/gi, '$1"$2": $3');
        ...
        if (errorPosition > 0 && errorPosition < jsonTextForFix.length) {
          ...
          while (i >= 0 && i > errorPosition - 50) {
            if (jsonTextForFix[i] === '"') {
              ...
              let j = i + 1;
              while (j < jsonTextForFix.length && /\s/.test(jsonTextForFix[j])) j++;
              if (j < jsonTextForFix.length && jsonTextForFix[j] !== ':') {
                // Missing colon - insert it
                jsonTextForFix = jsonTextForFix.substring(0, j) + ': ' + jsonTextForFix.substring(j);
                break;
              }
            }
            i--;
      ...
      if (isMissingCommaError) {
        ...
        if (errorPosition > 0 && errorPosition < jsonTextForFix.length) {
          ...
          for (let i = errorPosition - 1; i >= 0 && i > errorPosition - 50; i--) {
            const char = fixedJson[i];
            // If we find a quote, }, ], digit, or letter (part of true/false/null), insert comma after it
            if (char === '"' || char === '}' || char === ']' || /\d/.test(char) || /[a-z]/.test(char)) {
          ...
          if (insertPos < errorPosition && fixedJson[insertPos] !== ',') {
            ...
            try {
              const parsed = JSON.parse(fixedJson);
              console.log('✅ Successfully fixed and parsed JSON on first retry');
              return validateAndFormatLessonPlan(parsed, input);
            } catch (retryError) {
              // If quick fix didn't work, continue to full retry logic
            }
      ...
      console.warn('⚠️ JSON parsing issue detected (will attempt to fix):', parseError.message);
      ...
      if (errorPosition > 0 && errorPosition < responseText.length && process.env.DEBUG) {
      ...
      try {
        ...
        const parsed = JSON.parse(fixedJson);
        console.log('✅ Successfully fixed and parsed JSON on second attempt (initial parse had minor formatting issues)');
        return validateAndFormatLessonPlan(parsed, input);
        ...
        try {
          ...
          if (responseText && responseText.length > 0) {
            ...
            console.error('Response text (first 2000 chars):', responseText.substring(0, Math.min(2000, responseText.length)));
            ...
            if (errorPosition > 0 && errorPosition < responseText.length) {
              ...
              const problemEnd = Math.min(responseText.length, errorPosition + 100);
 ...
 */
const buildPrompt = (input: LessonPlanInput, fileContext: string, imageCount: number = 0): string => {
...
${imageCount > 0 ? `Có ${imageCount} hình ảnh đã được upload. Hãy xem kỹ các hình ảnh này để hiểu nội dung bài học cần soạn giáo án. Nếu hình ảnh chứa nội dung bài học, bài tập, hoặc tài liệu tham khảo, hãy sử dụng thông tin đó để soạn giáo án phù hợp.` : 'Không có hình ảnh được upload'}
...
const validateAndFormatLessonPlan = (
  ...
  if (parsed && (parsed.thong_tin_bai_hoc || parsed.yeu_cau_can_dat || parsed.hoat_dong_day_hoc)) {
    ...
    const toArray = (v: any): string[] => {
      const arr = Array.isArray(v) ? v.map((x) => String(x)) : typeof v === 'string' ? [v] : [];
    ...
    const addActivityToTable = (
      ...
      if (gv.length > 0 || hs.length > 0) {
        ...
        for (let i = 0; i < maxRows; i++) {
          ...
          if (gvLower.includes('mục tiêu') || gvLower.includes('cách tiến hành')) {
      ...
      const ketLuan = gv.find((text: string) => text.includes('GV Kết luận') || text.includes('Kết luận'));
      if (!ketLuan) {
    ...
    const formatTiet = (tietData: any, soTiet: number, totalTiets: number): { title: string; content: string } => {
      ...
      addActivityToTable(
        tableRows,
        tietData.hoat_dong_luyen_tap || tietData.hoat_dong_van_dung || tietData.hoatDongLuyenTap || tietData.hoatDongVanDung,
        `3. HOẠT ĐỘNG LUYỆN TẬP/VẬN DỤNG`
      );
    ...
    const soTiet = parsed.thong_tin_bai_hoc?.thoi_luong_tiet || Math.max(1, Math.round(input.duration / 35));
    const tietKeys = Object.keys(hd || {}).filter(key => key.startsWith('tiet_'));
    ...
    let activities: { activity1: { title: string; content: string }; activity2: { title: string; content: string }; activity3: { title: string; content: string }; activity4: { title: string; content: string } };
    ...
    if (tietKeys.length > 0) {
      ...
      if (formattedTiets.length > 4) {
        const remainingTiets = formattedTiets.slice(4).map(t => `${t.title}\n\n${t.content}`).join('\n\n---\n\n');
    ...
    if (!activities || 
        (!activities.activity1?.content && !activities.activity2?.content && 
         !activities.activity3?.content && !activities.activity4?.content)) {
      console.warn('⚠️ Không tìm thấy hoạt động dạy học, tạo activities mặc định');
