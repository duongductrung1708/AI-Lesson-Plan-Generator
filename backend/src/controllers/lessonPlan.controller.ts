  ...
  try {
    ...
    if (lessonPlan.uploadedFiles && lessonPlan.uploadedFiles.length > 0) {
      try {
        ...
        console.log(
          `Deleted ${lessonPlan.uploadedFiles.length} file(s) for lesson plan ${req.params.id}`
        );
      } catch (fileError: any) {
  ...
  try {
    ...
    const safeArray = (arr: any): string[] => {
      return Array.isArray(arr)
        ? arr.filter((item) => item && typeof item === "string")
        : [];
    };
    ...
    const competencies = objectives.competencies || {
      general: [],
      specific: [],
    };
    const equipment = content.equipment || { teacher: [], student: [] };
    const activities = content.activities || {};
    const adjustment = content.adjustment || {
      nhanXet: "",
      huongDieuChinh: [],
    };
        ...
        spacing: options?.spacing
          ? {
              before: options.spacing.before
                ? options.spacing.before * 20
                : undefined,
              after: options.spacing.after
                ? options.spacing.after * 20
                : undefined,
            }
          : undefined,
    ...
    const children: (Paragraph | Table)[] = [
      ...
      ...safeArray(competencies.specific).map((comp) =>
      ...
      ...safeArray(competencies.general).map((comp) =>
        createParagraph(comp.startsWith("-") ? comp : `- ${comp}`, {
          spacing: { after: 0.2 },
          indent: { left: 0.5 },
        })
      ...
      ...safeArray(objectives.qualities).map((quality) =>
      ...
      createParagraph(
        `- Giáo viên: ${safeArray(equipment.teacher).join(", ")}`,
        {
          spacing: { after: 0.5 },
        }
      ),
      createParagraph(
        `- Học sinh: ${safeArray(equipment.student).join(", ")}`,
        {
          spacing: { after: 0.5 },
        }
      ),
    ...
    const parseMarkdownTable = (tableLines: string[]): Table | null => {
      ...
      for (let i = 0; i < tableLines.length; i++) {
        ...
        if (shouldMergeRow && cells.length > 0) {
          ...
          const isBold =
            /^\d+\.\s*HOẠT ĐỘNG/i.test(mergedText) ||
    ...
    const parseTitle = (title: string): Paragraph => {
      if (!title)
        return createParagraph("", { spacing: { before: 1, after: 0.5 } });
    ...
    if (
      adjustment &&
      (adjustment.nhanXet ||
        (adjustment.huongDieuChinh && adjustment.huongDieuChinh.length > 0))
    ) {
      children.push(
        createParagraph("", { spacing: { after: 1 } }),
